...
export function renderTrack(group: ScrobbleGroup): string {
  ...
  const art = track.image ?? '/images/vinyl-placeholder.svg';
  return `
    <li class="track">
      <img class="track__art circle" src="${art}" alt="${escapeHTML(track.album)} cover" loading="lazy" width="44" height="44" />
      <div class="track__meta">
        <a class="link track__title" href="${track.trackUrl}" target="_blank" rel="noopener">${escapeHTML(track.track)}</a>
